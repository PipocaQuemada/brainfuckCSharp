...
public abstract record AST {
  ...
  public static StateT<Mem, IO, Unit> Step(AST ast) {
    return ast switch {
      Incr => StateT.modify<IO, Mem>(m => m with { Cur = (byte)(m.Cur + 1)}),
      Decr => StateT.modify<IO, Mem>(m => m with { Cur = (byte)(m.Cur - 1)}),
      Left => StateT.modify<IO, Mem>(m => m.MoveLeft()),
      Right => StateT.modify<IO, Mem>(m => m.MoveRight()),
      Write => StateT.gets<IO, Mem, byte>(m => m.Cur)
        .Bind((byte b) => StateT.liftIO<Mem, IO, Unit>(WriteByte(b))),
      Read => StateT.liftIO<Mem, IO, int>(ReadByte())
        .Bind(b => new Modify<Mem>(m => m with {Cur = (byte) b})),
      Loop(var body) => 
        from cur in StateT.gets<IO, Mem, byte>(m => m.Cur)
        from _1 in when(cur != 0, from _2 in Eval(body)
                                  from curLoopEnd in StateT.gets<IO, Mem, byte>(m => m.Cur)
